...
//       handleNext(3);
//     } else if (report.incidentInformation.reportType === "LR") {
//       handleNext(5);
//     } else {
//       handleNext(4);
//     }
...
const IncidentInfoForm: FC<IncidentInfoFormProps> = function ({ handleNext }) {
  ...
  /**
   * Handles form submission with dynamic step calculation
   * Validates form before proceeding to next step
   */
  function handleSubmit(event: React.FormEvent) {
    event.preventDefault();
    ...
    // Dynamic step calculation based on report type and ops center acknowledgment
    const isLR = report.incidentInformation.reportType === "LR";
    const isAcknowledged = report.incidentInformation.opsCenterAcknowledged;

    console.log('IncidentInfoForm submit - isLR:', isLR, 'isAcknowledged:', isAcknowledged);

    if (isAcknowledged) {
      // If ops center acknowledged, skip to step 3 (FirstFootageForm)
      // and set maxSteps to 4 (no LR forms needed)
      console.log('Ops center acknowledged - going to step 3, maxSteps 4');
      handleNext(4, 3);
    } else if (isLR) {
      // If LR and not acknowledged, proceed normally
      // The AddEntryPage will calculate the correct maxSteps based on quantities
      console.log('LR and not acknowledged - proceeding normally');
      handleNext();
    } else {
      // If LA (Late Activation) and not acknowledged, proceed normally with standard flow
      console.log('LA and not acknowledged - standard flow, maxSteps 4');
      handleNext(4);
    }
  }
