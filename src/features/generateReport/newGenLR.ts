...
const getLRTableData = function (
  ...
  if (type === "First") {
    ...
    const tableData = [
      ...
      { value: `${generalInformation.boundary} Mins Boundary (${shortLongStation[incidentInformation.turnoutFrom]})`, color: colors.black },
      { value: `${incidentInformation.SC} / ${incidentInformation.PO}`, color: colors.black, colspan: 2 },
    ];

    return tableData.map(({ value, color, colspan }) => ({
      text: value,
      options: { fontSize: 9, fill: colors.white, color, ...(colspan && { colspan }) },
    }));
  }
...
const generateLrReport = async function (pptx: PptxGenJS, report: Report) {
  ...
  first.addTable(
    [
      tableHeaders.legends,
      ...legendsTableDetails.details
    ],
